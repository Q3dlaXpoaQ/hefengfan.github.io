...
Search.prototype.searchFull = function(vlAlpha_, vlBeta, depth, noNull,x) {
  var vlAlpha = vlAlpha_;
  if (depth <= 0) {
  ...
  if (!noNull && !this.pos.inCheck() && this.pos.nullOkay() && !x) {
    this.pos.nullMove();
...
Search.prototype.searchRoot = function(depth,x) {
  var vlBest = -MATE_VALUE;
  ...
  while ((mv = sort.next()) > 0) {
    ...
    if (vlBest == -MATE_VALUE) {
      ...
      vl = -this.searchFull(-vlBest - 1, -vlBest, newDepth, false,mv==x?true:null);
      if (vl > vlBest) {
        vl = -this.searchFull(-MATE_VALUE, -vlBest, newDepth, true);
      }
    }
    this.pos.undoMakeMove();
    if (x && x!=0 && x==mv && vl > -WIN_VALUE && vl < WIN_VALUE){
          vl += Math.floor(Math.random() * RANDOMNESS)*5
            vl = (vl == this.pos.drawValue() ? vl - 1 : vl);
      }
    if (vl > vlBest) {
      vlBest = vl;
      this.mvResult = mv;
  ...
  this.recordHash(HASH_PV, vlBest, depth, this.mvResult);
...
Search.prototype.searchMain = function(depth, millis,x) {
  ...
  var html1='';
    document.getElementById('s').innerHTML='<div style="text-align:center;font-size:15px;">深度:'+i+' 分数:'+vl+' 招法:'+board.pos.convertMvToCn(this.mvResult)+' '+Math.ceil(this.getKNPS())+'K</div>';
    document.getElementById('m').innerHTML=html1;
  return this.mvResult;
}
